...
%             end
        ...
        function startGame(self)
            ...
            self.Player2UI.head([102,100,101]);   
            self.Player3UI.head([101,102,100]);
      ...
      %%  
        ...
        function loop(self,pass,kind)   
            noskip=false;
            if pass
                self.passtimes=self.passtimes+1;
            else
                if kind==9 || kind==7 || kind==8
                    self.Player1UI.showEffect(kind);
                    self.Player2UI.showEffect(kind);
                    self.Player3UI.showEffect(kind)
                end    
                self.passtimes=0;     
            end
            p1=self.cards1;
            ...
            if ~isempty(p1) && ~isempty(p2) && ~isempty(p3)
                ...
                self.getNowSplit(noskip);        %enable nextplayers' cards button
        ...
        function gameover(self)
            ...
            self.Player1UI.gameover(winnerSTR,winnerACT);
            self.Player2UI.gameover(winnerSTR,winnerACT);
            self.Player3UI.gameover(winnerSTR,winnerACT);
      ...
      function  [newp]=deleteCard(self,p,nowsplit)
       ...
       function getNowSplit(self,noskip)  
